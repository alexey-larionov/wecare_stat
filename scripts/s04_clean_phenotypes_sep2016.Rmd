---
title: "Clean phenotypes: wecare feb2016"
...
interim_data_file <- paste(interim_data_folder, "/", "s03_filter_by_effect_", prefix, "_", variants_set,".RData", sep="")
...
variants.df <- 
  ...
  select(SplitVarID, SYMBOL, TYPE, CHROM, POS, REF, ALT, AC, AF, AN, 
         ...
         Existing_variation, Multiallelic, priority_genes)
...
# explore_and_reshape_covar_table

```{r explore_and_reshape_covar_table}
...
# family_history split 158 vs 340, no missed data
sum(family_history)
...
# Get positions and IDs of duplicated cases
duplicated_cases_bool <- duplicated(pheno.df$Subject_ID) # Excluding position of the 1st duplicate
duplicated_cases_ids <- as.vector(pheno.df$Subject_ID[duplicated_cases_bool])

# Phenotype annotations for the duplicate record are identical
pheno.df %>% 
  filter(Subject_ID %in% duplicated_cases_ids) %>% 
  select(1:5)

# Remove the duplicated cases (except the 1st duplicate)
pheno.df <- pheno.df[!duplicated_cases_bool,]
...
# Clean-up
rm(duplicated_cases_bool, duplicated_cases_ids)

# Attach pheno for further exploration
attach(pheno.df)
...
# Compare chemo and CMF
pheno.df[1:5,c("chemo", "CMF")]
...
pheno_table_file <- paste(interim_data_folder, "/", "s03_filter_by_effect_", prefix, "_", variants_set,"_pheno.txt", sep="")
write.table(phenotypes.df, file=pheno_table_file, quote=FALSE, sep="\t")

rm(samples.df, phen_types.df, pheno_table_file)
...
save.image(paste(interim_data_folder, "/", "s04_clean_phenotypes", prefix, "_", variants_set,".RData", sep=""))
